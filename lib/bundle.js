...
/***/ function(module, exports, __webpack_require__) {
	...
	var Game = function () {
	  ...
	  _createClass(Game, [{
	    ...
	    key: "addFilledPoints",
	    value: function addFilledPoints(shapeArray) {
	      ...
	      for (var x = x_min; x <= x_max; x++) {
	        for (var y = y_min; y <= y_max; y++) {
	          ...
	          if (this.rayCasting(pos, shapeArray.slice(1))) {
	            this.filledPositions.add(pos.toString());
	          }
	        }
	      }
	    }
	    ...
	    value: function bounce(pos) {
	      if (pos[0] <= 0 || pos[0] >= Game.DIM_X || pos[1] <= 0 || pos[1] >= Game.DIM_Y || this.filledPositions.has(pos.toString())) {
	        return true;
	      }
	      return false;
	    }
	...
	var MovingObject = function () {
	  ...
	  _createClass(MovingObject, [{
	    ...
	    value: function move(timeDelta) {
	      ...
	      var next_pos = [Math.round(this.pos[0] + offsetX), Math.round(this.pos[1] + offsetY)];
	      var x_vel = this.vel[0];
	      var y_vel = this.vel[1];
	
	      if (this.game.hitWall(next_pos)) {
	        var next_vel = this.checkRebound(next_pos, this.vel);
	        if (next_vel === "x") {
	    ...
	    key: "findCirclePoints",
	    value: function findCirclePoints(pos) {
	      var min_x = pos[0] - this.radius;
	      var max_x = pos[0] + this.radius;
	      var min_y = pos[1] - this.radius;
	      var max_y = pos[1] + this.radius;
	    ...
	    key: "pointInCircle",
	    value: function pointInCircle(pos) {
	      var min_x = pos[0] - this.radius;
	      var max_x = pos[0] + this.radius;
	      var min_y = pos[1] - this.radius;
	      var max_y = pos[1] + this.radius;
