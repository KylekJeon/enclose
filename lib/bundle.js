	...
	var Game = function () {
	  ...
	  _createClass(Game, [{
	    ...
	    value: function draw(ctx) {
	      ...
	      var percent = this.findFilledPercent(ctx);
	    }
	  }, {
	      ...
	      if (seen.length > 1) {
	        ...
	        if (this.borderPositions.has(seen[seen.length - 1].toString())) {
	          ...
	          seen.forEach(function (pos) {
	            _this.borderPositions.add(pos.toString());
	            _this.emptyPositions.delete(pos.toString());
	...
	var Ship = function (_MovingObject) {
	  ...
	  _createClass(Ship, [{
	    ...
	    value: function power(impulse) {
	      ...
	      if (!this.started) {
	        if (this.game.borderPositions.has(current_pos.toString()) && !this.game.borderPositions.has(next_pos.toString())) {
	          this.started = true;
	          this.game.seenPositions.push(current_pos);
	          this.game.seenPositions.push(next_pos);
	        }
	      } else {
	        if (this.game.borderPositions.has(next_pos.toString())) {
	          this.started = false;
	        }
	        this.game.seenPositions.push(next_pos);
	      }
